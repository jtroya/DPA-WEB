...
<?php
	...
	//Funcion para consultar la taxonomia de tipo de profesor
	function get_tipo_vinculacion () {
		$categorias_profe = get_the_terms($post->ID, 'tipo-profesor');
		if ( $categorias_profe && ! is_wp_error( $categorias_profe ) ) {
			$vinculacion = array();
			foreach ($categorias_profe as $cat_profe) {
				$vinculacion[] = $cat_profe->name;
			}
			$tipo_vinculacion = join(" <br> ", $vinculacion);
			//logConsole('Funcion get tipo vinculacion ', $tipo_vinculacion);
		} 
						...
						<article>
							<div class="row-fluid">
								...
								<div class="span8 inv-responsable">
									...
									<p><?php echo get_tipo_vinculacion(); ?></p>
								</div>
							</div>
						</article>
					</div>
